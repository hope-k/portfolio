...
const Home = () => {
  ...
  useLayoutEffect(() => {
    ...
    t1.from('#line1', {width: 0, ease: 'sine.in', duration: .8})
      ...
      .from(text2.chars, { color: '' y: 10, opacity: 0, scale: 1.6, ease: "sine.out", stagger: { each: 0.09, from: 'start' }, duration: .8 }, '-=.8')
      ...
      .from('#line2', {width: 0, ease:'sine.in', duration: .8, borderTopColor: 'teal' })
  },[])
      ...
      <Flex mt={['6rem', '0']} alignItems={['flex-start', 'flex-start', 'center']} w='100%' flexDir={['column', 'column', 'row']} fontFamily={'rale'}>
        <Box mb={['3rem', '4rem', '0']} color='white' d='flex' flexDir={'column'} justifyContent={'center'} width={'100%'} position={'relative'}>
          ...
          <Box display={['none', 'inline-block']}>
            <hr id='line2' style={{ display: 'inline-block', width: '20rem', position: 'absolute', bottom: '-1rem', left: '5rem', borderTop: '2px solid yellow' }} />
          </Box>
        </Box>
        <Box d='inline-block'>
          <Button id='buttonTwo' transition={'all .25s ease'} mb='1rem' ml={['0', '2rem', '0', '0']} _hover={{ bg: 'purple.100', pl: '2rem', color: 'black' }} color='white' variant={'outline'} borderColor={'purple.100'}>MORE ABOUT ME</Button>
          <Button id='buttonOne' transition={'all .25s ease'} mb='1rem' _hover={{ bg: 'teal', pl: '2rem' }} borderColor={'teal'} color='white' variant={'outline'}>LATEST PROJECTS</Button>
        </Box>
      </Flex>
    </Container>

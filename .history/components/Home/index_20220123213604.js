...
const Home = () => {
  ...
  useEffect(() => {
    ...
    t1.from(text1.chars, { y: -20, opacity: 0, ease: 'Bounce.easeOut', stagger: { each: 0.1, from: 'start' } })
      .from(text2.chars, {y: 10, opacity: 0, ease: "slow(0.5, 0.8)", stagger: {each: 0.09, from: 'start'}, duration: .8 }, '-=.8')
      .from(text3.chars, { x: -60, opacity: 0, ease: 'slow(1, 2)', stagger: {each: 0.07, from: 'random'} })
  })

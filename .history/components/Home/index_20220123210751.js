...
const Home = () => {
  ...
  useEffect(() => {
    ...
    t1.from(text.chars, { x: 30, opacity: 0, scaleX: .2, ease: 'power4.easeOut', stagger: { each: 0.03 } })
  })
    ...
    <Container d='flex' justifyContent='center' alignItems={'center'} maxW={['21rem', '30rem', '70rem']} h='100vh'>
      <Flex mt={['6rem', '0']} alignItems={['flex-start', 'flex-start', 'center']} w='100%' flexDir={['column', 'column', 'row']} fontFamily={'rale'}>
        <Box mb={['3rem', '4rem', '0']} color='white' d='flex' flexDir={'column'} justifyContent={'center'} width={'100%'} position={'relative'}>
          <Text maxW={'100%'} color='teal' fontSize='1.4rem' ml='2rem'><hr style={{ display: 'inline-block', width: '2rem', position: 'absolute' }} ref={textRef} />HELLO THERE,</Text>
          <Text fontSize={['2.3rem', '4rem']} mt={['1rem', '2rem', '0rem']} ref={textRef}> I am <b>Hope Kumordzie.</b> </Text>
          <Text fontWeight={['500', '600']} fontSize={'2.5rem'} ref={textRef}> A FullStack Web Developer </Text>

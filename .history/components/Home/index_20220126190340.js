...
const Home = () => {
  ...
  useLayoutEffect(() => {
    ...
    t1.from('#line1', { width: 0, ease: 'sine.in', duration: .8 })
      ...
      .from('#buttonOne', { y: -7, opacity: 0, ease: 'bounce.in' }, '-=1' )
      .from('#buttonTwo', { y: 7, opacity: 0, ease: 'bounce.in' }, '-=1')
      .from('#line2', { width: 0, borderTop: '0px solid teal', ease: 'sine.in', duration: .8, borderTopColor: 'teal' }, '-=1')
  }, [])
        ...
        <Flex ref={panelContainer} w='400%' h='100%' flexWrap={'nowrap'}>
            ...
            <Container d='flex' justifyContent='center' alignItems={'center'} maxW={['21rem', '30rem', '70rem']} h='100vh'>
              <Flex mt={['6rem', '0']} alignItems={['flex-start', 'flex-start', 'center']} w='100%' flexDir={['column', 'column', 'row']} fontFamily={'rale'}>
                <Box mb={['3rem', '4rem', '0']} color='white' d='flex' flexDir={'column'} justifyContent={'center'} width={'100%'} position={'relative'}>
                  ...
                  <Text color='#ccc' fontWeight={['500', '600']} fontSize={'2.5rem'} ref={titleRef}> A FullStack Web Developer </Text>
          ...
          <Box id='panel' d='flex' position={'relative'} h='100%' w='100%' bg='rgba(0 ,0 ,0, .87)'>
            ...
            <Flex display={['none', 'flex']} position={'absolute'} height={'100vh'} width='10%' right={'0'} bg='rgba(0, 0, 0, .2)' justifyContent={'center'} alignItems={'center'}>
              <Text ml='3rem' fontSize={'4rem'} letterSpacing={'1rem'} fontFamily={'rale'} fontWeight={'900'} transform={'rotate(270deg)'} color='rgba(192,192,192,.025)' >
                WORKS 

...
const Home = () => {
  ...
  useEffect(() => {
    ...
    t1.from(text1.chars, { x: -5, opacity: 0, ease: 'Power0.ease', stagger: { each: 0.1, from: 'start' } })
      .from(text2.chars, {y: 10, opacity: 0, ease: 'Power0.ease', stagger: {each: 0.09, from: 'start'}, duration: .8 }, '-=.8')
      .from(text3.chars, {x: -60, opacity: 0, ease: 'Bounce.easeout', stagger: {each: 0.07, from} })
  })

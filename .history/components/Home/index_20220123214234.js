...
const Home = () => {
  ...
  useEffect(() => {
    ...
    t1.from(text1.chars, { x: -5, opacity: 0, ease: 'Power0.ease', stagger: { each: 0.1, from: 'start' } })
      .from(text2.chars, {y: 10, opacity: 0, scale: .6, ease: "sine.out", stagger: {each: 0.09, from: 'start'}, duration: .8 }, '-=.8')
      .from(text3.chars, { y: -60, opacity: 0, ease: 'circ.out', stagger: {each: 0.1, from: 'random'} })
  })
    ...
    <Container d='flex' justifyContent='center' alignItems={'center'} maxW={['21rem', '30rem', '70rem']} h='100vh'>
      <Flex mt={['6rem', '0']} alignItems={['flex-start', 'flex-start', 'center']} w='100%' flexDir={['column', 'column', 'row']} fontFamily={'rale'}>
        ...
        <Box transition={'all 1s ease-in'} d='inline-block'>
          ...
          <Button id='button' transition={'all .45s ease'} mb='1rem' ml={['0', '2rem', '0', '0']} _hover={{ bg: 'purple.100', pl: '2rem', color: 'black' }} color='white' variant={'outline'} borderColor={'purple.100'}>MORE ABOUT ME</Button>
        </Box>
      </Flex>
    </Container>

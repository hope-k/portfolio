...
const Home = () => {
  ...
  useLayoutEffect(() => {
    ...
    t1.from('#line1', { width: 0, ease: 'sine.in', duration: .8 })
      .from(text1.chars, { duration: 8, x: -15, opacity: 0, ease: 'Power0.ease', stagger: { each: 0.2, from: 'start' } })
      .from(text2.chars, { color: 'teal', y: 10, opacity: 0, scale: 1.6, ease: "sine.out", stagger: { each: 0.09, from: 'start' }, duration: .8 }, '-9')
      .from(text3.chars, { color: 'teal', y: -60, opacity: 0, ease: 'bounce.out', stagger: { each: 0.1, from: 'random' } }, '-=8')
      .from('#buttonOne', { y: -6, opacity: 0, ease: 'Power4.inOut' }, '-=6')
      .from('#buttonTwo', { y: 6, opacity: 0, ease: 'bounce.in' }, '-=6')

...
const Home = () => {
  ...
  return (
    ...
    <>
      ...
      <Box position={'relative'} ref={bgTrigger} id='home'>
        ...
        <Container d='flex' justifyContent='center' alignItems={'center'} maxW={['21rem', '30rem', '70rem']} h='100vh'>
          <Flex mt={['6rem', '0']} alignItems={['flex-start', 'flex-start', 'center']} w='100%' flexDir={['column', 'column', 'row']} fontFamily={'rale'}>
            <Box lineHeight={'1'} mb={['3rem', '4rem', '0']} color='white' d='flex' flexDir={'column'} justifyContent={'center'} width={'100%'} position={'relative'}>
              ...
              <Text letterSpacing={'-5.5px'} textTransform={'none'} fontFamily={'Montserrat'} fontWeight={['400', '600']} fontSize={['2.3rem', '5.5rem']} mt={['1rem', '2rem', '0rem']} ref={nameRef}><h1>I&apos;m Hope Kumordzie.</h1></Text>
      ...
      <Box d='flex' position={'relative'} h='93vh' w='100%' bg='rgba(0 ,0 ,0, .87)'>
        <About />
      </Box>



      <Box d='flex' position={'relative'} bg='rgba(0 ,0 ,0, .87)' h='100vh' w='100%'>
        <Center justifyContent={'center'} alignItems={'center'} m='auto' position={'absolute'} width={'100%'} h='7vh' top={'0'} bg='rgba(0, 0, 0, .2)' >

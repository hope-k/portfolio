...
const WorkItem = ({ title, description, laptopMock, phoneMock, phoneImage, link }) => {
    ...
    return (
        <Container background={'rgba(36, 36, 36, .35)'} zIndex={'1'} d='flex' m='1rem' h='90vh' w='100vw'>
            <Box>
                ...
                <Flex w='100%' id='workTile' flexDir={'column'} alignItems={'center'} justifyContent={'center'}>
                    <Text color='#ccc' pb='1rem' mb='1rem' style={{ borderBottom: '2px solid teal', borderBottomWidth: '1px' }}>{title}</Text>
                    <Text className='outfit' color='#ccc' pb='2rem' maxW={'40ch'}  textTransform={'capitalize'}>{description}</Text>
                    <Box m='2rem' position='absolute' bottom='0' d='flex' justifyContent={'center'} alignItems={'center'}>
                        <Link href={link} passHref>
                            <Button w='100%' colorScheme={'teal'} variant={'outline'}>View Project</Button>
                        </Link>
                    </Box>
                </Flex>

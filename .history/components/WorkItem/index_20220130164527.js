...
const WorkItem = ({ title, description, laptopMock, phoneMock, phoneImage, link }) => {
    return (
        <Container background={'rgba(36, 36, 36, .85)'} d='flex'>
            <Box>
                ...
                <Flex flexDir={'column'}  alignItems={'center'} justifyContent={'center'}>
                    <Text pb='1rem'>{title}</Text>
                    <Text COLOR='' pb='2rem' maxW={'40ch'}>{description}</Text>
                    <Link href={link} passHref>
                        <Button w='80%' colorScheme={'teal'} variant={'outline'}>View Project</Button>
                    </Link>
                </Flex>
            </Box>
        </Container>

...
const WorkItem = ({ title, description, laptopMock, phoneMock, phoneImage, link }) => {
    const [openTile, setOpenTile] = useState(true);
    const toggleOpenTile = () => setOpenTile(!openTile)
    const workTile = useRef(null);
    const t2 = useRef(null);
    useLayoutEffect(() => {
        t2.current = gsap.timeline({ defaults: { paused: true } })
    ...
    return (
        <Container background={'rgba(36, 36, 36, .35)'} zIndex={'1'} d='flex' m='1rem'>
            <Box>
                ...
                <Flex id='workTile' ref={workTile} flexDir={'column'} alignItems={'center'} justifyContent={'center'}>

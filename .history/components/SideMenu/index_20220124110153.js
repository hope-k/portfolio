...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        t1.from(menuRef.current, { width: '0', ease: 'power3.inOut' })
    })
    return (
        <>
            {
                open ? (
                    <>
                        <Box ref={menuRef} id='sideMenu' display={['flex', 'none']} alignItems={'center'} justifyContent={'center'} d='flex' position={'absolute'} zIndex={'9'} top={0} bottom={0} left={0} right={0} height={'100vh'} width={'100%'} background={'rgba(36, 36, 36, 1)'}>
                            <Box d='flex' >

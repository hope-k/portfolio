...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        t1.current = gsap.timeline({ paused: true })
            .to('#sideMenu', { left: '0rem', ease: 'power3.inOut', duration: 1.25 });
    ...
    return (
        ...
        <>
            <Box id='sideMenu' opacity0={`${open ? 'visible' : 'hidden'}`} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} left={0} right={0} height={'100vh'} width={'100%'} background={'rgba(36, 36, 36, 1)'}>
                <Box d='flex'>

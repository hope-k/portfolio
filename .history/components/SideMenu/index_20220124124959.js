...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        const t2 = gsap.timeline({defaults: {duration: .25, stagger: 0.003}});
        t2.from('#stack', { x: 21, opacity: 0 })
            .from(link1.words, { x: 20, opacity: 0, ease: 'power4.out', duration: .8 })
            .from(link2.words, { x: 20, opacity: 0, ease: 'power4.out', duration: .8})
            .from(link3.words, { x: 20, opacity: 0, ease: 'power4.out', duration: .8 })
            .from(link4.words, { x: 20, opacity: 0, ease: 'power4.out', duration: .8})

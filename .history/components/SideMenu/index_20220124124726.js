...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        const t2 = gsap.timeline({defaults: {duration: .25, stagger: .23}});
        t2.from('#stack', { x: 21, opacity: 0 })
            .from(link1.chars, { x: 20, opacity: 0, ease: 'elastic.out', })
            .from(link2.chars, { x: 20, opacity: 0, ease: 'elastic.out', })
            .from(link3.chars, { x: 20, opacity: 0, ease: 'elastic.out',  })
            .from(link4.chars, { x: 20, opacity: 0, ease: 'elastic.out', })

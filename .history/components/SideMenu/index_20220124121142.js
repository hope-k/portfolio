...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        t1.current = gsap.timeline({paused: true})
            .to('#sideMenu', { left: '0', ease: 'elastice.in', duration: 1 });
    ...
    return (
        ...
        <>
            <Box id='sideMenu' display={['flex', 'none']} opacity={'1'} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} left={'30rem'} right={0} height={'100vh'} width={'100%'} background={'rgba(36, 36, 36, 1)'}>
                <Box d='flex'>

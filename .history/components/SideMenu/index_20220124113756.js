...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        t1.to('#sideMenu', { left: 0, ease: 'power3.inOut'})
    })
    return (


        <>
            <Box id='sideMenu' visibility={`${open ? 'visible' : 'invisible'}`} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} left={0} right={0} height={'100vh'} width={'100%'} background={'rgba(36, 36, 36, 1)'}>
                <Box d='flex'>

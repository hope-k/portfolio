...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        const t2 = gsap.timeline({defaults: {duration: .15}});
        t2.from('#stack', { x: 21, opacity: 0 })
            .from(link1.chars, { x: 20, opacity: 0, ease: 'power4.out', stagger: { each: 0.01 } })
            .from(link2.chars, { x: 20, opacity: 0, ease: 'power4.out', stagger: { each: 0.03 } })
            .from(link3.chars, { x: 20, opacity: 0, ease: 'power4.out', stagger: { each: 0.03 } })
            .from(link4.chars, { x: 20, opacity: 0, ease: 'power4.out', stagger: { each: 0.03 } })

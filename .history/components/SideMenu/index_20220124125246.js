...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        ...
        t2.from('#stack', { x: 21, opacity: 0, ease: 'power4.out', stagger: {each: .03} })
            .from('#links', { x: 20, opacity: 0, ease: 'power4.out', duration: .8 })
            .from('#links', { x: 20, opacity: 0, ease: 'power4.out', duration: .8})
            .from('#links', { x: 20, opacity: 0, ease: 'power4.out', duration: .8 })
            .from('#links', { x: 20, opacity: 0, ease: 'power4.out', duration: .8})
            ...
            <Box id='sideMenu' display={['flex', 'none']} opacity={'1'} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} right={'-15rem'} height={'100vh'} background={'rgba(36, 36, 36, .97)'}>
                <Box d='flex'>
                    <Stack id='stack' color='#ccc' direction={'column'} fontFamily={'rale'} fontSize={'2.5rem'} letterSpacing={'4px'} >
                        <Text id='links' ref={linkRef1} transition='all .25s ease' _hover={{ pl: '1rem', color: 'purple.400' }}>Home</Text>
                        <Text id='links' ref={linkRef2} transition='all .25s ease' _hover={{ pl: '1rem', color: 'purple.400' }}>Works</Text>
                        <Text id='links' ref={linkRef3} transition='all .25s ease' _hover={{ pl: '1rem', color: 'purple.400' }}>About</Text>
                        <Text id='links' ref={linkRef4} transition='all .25s ease' _hover={{ pl: '1rem', color: 'purple.400' }}>Contact</Text>
                    </Stack>
                </Box>
            </Box>
        </>

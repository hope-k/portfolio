...
const SideMenu = ({ open, toggleOpen}) => {
    ...
    useLayoutEffect(() => {
        ...
        t1.current = gsap.timeline({ paused: true, defaults: { duration: .65 } })
            .to('#sideMenu', { left: 0, right: 0, opacity: 1, ease: 'Power4.easeInOut' })
            .from('#menuLink', { duration: .68, opacity: 0, y: 11, ease: 'Power4.easeInOut', stagger: { each: 0.15 } }, '-=.64')
    ...
    return (
        ...
        <>
            ...
            <Box display={['flex', 'none']} id='sideMenu' opacity={0} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} right={'-15rem'} height={'100vh'} background={'rgba(36, 36, 36, .98)'}>
                <Box d='flex'>
                    ...
                    <Box bg='blackAlpha.300' position={'absolute'} bottom='0' h='5vh' w='100vw' left='0'>
                        <Center h='100%' d='flex' justifyContent={'center'} alignItems={'center'}>
                            <Text color='whiteAlpha.100'>Hope Kumordzie &copy;</Text>
                        </Center>
                    </Box>
                </Box>

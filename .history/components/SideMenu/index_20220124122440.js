...
const SideMenu = ({ open }) => {
    ...
    useLayoutEffect(() => {
        t1.current = gsap.timeline({paused: true})
            .to('#sideMenu', { left: '0', right: 0, opacity: '1', ease: 'power4.inOut', duration: .8 });
    ...
    return (
        ...
        <>
            <Box id='sideMenu' display={['flex', 'none']} opacity={'1'} alignItems={'center'} justifyContent={'center'} d='flex' position='fixed' zIndex={'9'} top={0} bottom={0} right={'-15rem'}  height={'100vh'}  background={'rgba(36, 36, 36, 1)'}>
                <Box d='flex'>

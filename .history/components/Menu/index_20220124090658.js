...
const MenuItem = styled.div`
    ...
    display: block;
    margin-bottom: 4px;
    transition: all .s cubic-bezier(.215, .61, .355, 1);
    &:nth-child(1){
        ${({ open }) => open && 'left: 0px'};
...
const Menu = ({ open }) => {
    useLayoutEffect(() => {
        ...
        const t1 = gsap.timeline({ defaults: { duration: .6, delay: 2 } });
        t1.from('#menu', { x: -15, opacity: 0, ease: 'Power0.ease', stagger: { each: 0.2, from: 'start' } })
    })
    return (
        <>

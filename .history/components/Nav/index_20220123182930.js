...
const NavBar = () => {
    ...
    return (
        <Box position='fixed' d='flex' top={0} background={'rgba(51, 54, 52, .6)'} w={'100%'}>
            <Container maxW={['21rem','90rem']} p={'3'}>
                <Flex alignItems={'center'} justifyContent={'space-between'}>
                    <Box>
                        <Text fontSize={30} color='#ccc' fontWeight='bold'>HK</Text>
                    </Box>
                    <Flex color='white'>
                        <Flex display={['none', 'inline-block']}}>
                        ...
                        <Box onClick={() => toggleOpen()} position={'relative'} top={'5px'} cursor={'pointer'} transition={'transition: all .5s cubic-bezier(.215, .61, .355, 1)'}>

import React, { useState, useLayoutEffect, useE } from 'react';
...
const NavBar = () => {
    ...
    useLayoutEffect(() => {
        ...
        t1.from('#nav', {top: '-62px', ease: 'Power4.ease'})
          .from('#logo', {x:-30, opacity: 0, ease: 'bounce.out'})
        ...
        <Box id='nav' position='fixed' d='flex' top={0} background={'rgba(0, 14, 12, .6)'} w={'100%'}>
            <Container  maxW={['21rem','40rem', '90rem']} p={'3'}>
                <Flex alignItems={'center'} justifyContent={'space-between'}>
                    <Box>
                        <Text id='logo' fontSize={[25,30]} color='#ccc' fontWeight='bold'>HK</Text>
                    </Box>
                    <Flex color='white'>

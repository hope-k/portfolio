...
const NavBar = ({ aboutVisible }) => {
    ...
    useLayoutEffect(() => {
        ...
        t1.from('#nav', { top: '-65px', opacity: 0, ease: 'Power4.ease' })
            .from('#links', { opacity: 0, duration: 1.8, x: 10, ease: 'Expo.easeOut', stagger: { each: 0.15 } })
    ...
    return (
        <>
            <Box className='poiret' id='nav' position='absolute' d='flex' left='0' top={0} background={['rgba(0, 0, 0, .95)', 'rgba(0, 0, 0, .8)']} w={'100%'} zIndex={10}>
                        ...
                        <Flex color='white'>
                            <Flex display={['none', 'inline-flex']}>
                                ...
                                <NextLink href='/#works' scroll={false}>
                                    <Text id='links' _hover={{ color: 'purple.300' }} transition={'all .4s ease'} mr={'6'} letterSpacing={'1.7px'} cursor={'pointer'} color={router.pathname.includes('/#works') ? 'teal' : 'white'}>Works</Text>
                                </NextLink>
                                <NextLink href='/#contact' scroll={false}>
                                    <Text id='links' _hover={{ color: 'purple.300' }} transition={'all .4s ease'} mr={'6'} letterSpacing={'1.7px'} cursor={'pointer'} color={router.pathname === '/#contact' ? 'teal' : 'white'}>Contact Me</Text>
                                </NextLink>
                            </Flex>

...
const NavBar = () => {
    ...
    useLayoutEffect(() => {
        ...
        t1.from('#nav', { top: '-65px', opacity: 0, ease: 'Power4.ease' })
            .from('#links', { opacity: 0, duration: .8, scale: .8, ease: 'cubic-bezier(.6, .01, -.05, .95', stagger: { each: 0.3 } })
            .from(logoSplit.chars, { opacity: 0, scale: 2.5, ease: 'bounce.out', stagger: { each: 0.6 } }, '-=.4')


    }, [])
    useLayoutEffect(() => {
        const navAnimation = gsap.from('#',{
            yPercent: -100,
            paused: true,
            duration: .8,
        ...
        ScrollTrigger.create({
            ...
            onUpdate: (self) => {
                self.direction === -1 ? navAnimation.play() : navAnimation
            }
        })
    })
    ...
    return (
        <>
            <Box id='nav' position='fixed' d='flex' left='0' top={0} background={['rgba(0, 0, 0, .95)', 'rgba(0, 0, 0, .8)']} w={'100%'} zIndex={10}>

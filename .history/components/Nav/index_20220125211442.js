...
const NavBar = () => {
    ...
    useLayoutEffect(() => {
        ...
        t1.from('#nav', { top: '-65px', opacity: 0, ease: 'Power4.ease' })
            .from('#links', { opacity: 0, duration: .8, scale: .8, ease: 'sine.out', stagger: { each: 0.3 } })
    ...
    return (
        <>
            <Box id='nav' position='fixed' d='flex' top={0} background={['rgba(0, 0, 0, .95)', 'rgba(0, 0, 0, .1)']} w={'100%'} zIndex={10}>

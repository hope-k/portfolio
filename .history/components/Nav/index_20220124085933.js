...
const NavBar = () => {
    ...
    t1.from('#nav', { top: '-62px', ease: 'Power4.ease' })
        .from(linksRef.current, { opacity: 0, scale: .8 ,ease: 'sine.in', stagger: { each: 0.5 } })
        .from(logoSplit.chars, { opacity: 0, scale: 2.5, ease: 'bounce.out', stagger: { each: 0.6 } })
            ...
            <Container maxW={['21rem', '40rem', '90rem']} p={'3'}>
                <Flex alignItems={'center'} justifyContent={'space-between'}>
                    ...
                    <Flex color='white'>
                        ...
                        <Box ref={linksRef} display={['inline-flex', 'none']} onClick={() => toggleOpen()} position={'relative'} top={'5px'} cursor={'pointer'} transition={'transition: all .5s cubic-bezier(.215, .61, .355, 1)'}>
